...
import { PAGE_SIZE_OPTIONS, PAGE_SIZE, JPG_EXTENTION } from 'src/app/shared';
...
@Component({
    templateUrl: './remittance-disburse.component.html',
    styleUrls: ['./remittance-disburse.component.css']
})
export class RemittanceDisburseComponent implements OnInit {

    displayedColumns = ['index', 'recipientName', 'senderCountry', 'exchangeHouse', 'actualAmt', 'pin', 'mobile', 'photoId', 'action'];
    ...
    isLoading; showControlNumber; frontPhotoModalDialog: boolean;
    traceId; oid; actualAmountInBdt; photoIdFrontPhoto: any;

    constructor(private authService: AuthService, private remittanceService: RemittanceDisburseService, private messagesService: GrowlMessageService,
    ...
    showPhotoId(ele){
       this.frontPhotoModalDialog =true;
       this.photoIdFrontPhoto = IFR_IMAGE_PATH + ele.photoIdFrontPath.slice(ele.photoIdFrontPath.lastIndexOf("/") + 1, ele.photoIdFrontPath.lastIndexOf(".")) + JPG_EXTENTION;
    }

    close() {
        this.showControlNumber = false;
        this.remittanceService.disburse(this.oid, this.userInfo.loginId).subscribe(result => {
    ...
    onDisburseClick(ele) {
        ...
        this.traceId = ele.traceId;
        this.actualAmountInBdt = ele.actualAmountInBdt;
        this.oid = ele.oid;
    ...
    sortData(sort: Sort) {
        ...
        this.dataSource = data.sort((a, b) => {
            ...
            switch (sort.active) {
             ...
             case 'exchangeHouse': return compare(+a.exHouseName, +b.exHouseName, isAsc);
