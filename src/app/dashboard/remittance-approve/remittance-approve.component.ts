...
@Component({
  templateUrl: './remittance-approve.component.html',
  styleUrls: ['./remittance-approve.component.css']
})

export class RemittanceApproveComponent implements OnInit {

  displayedColumns = ['select', 'lock', 'senderName', 'pin', 'senderCountry', 'recipientName',  'mobile', 'amount', 'actualAmt', 'lockedBy', 'applicationDate', 'status'];
  ...
  showDialog; showRemarks; isLoading : boolean;
  remarks:string;
  

  constructor(private authService: AuthService, private remittanceService: RemittanceApproveService, private messagesService: GrowlMessageService,
 ...
 lockNew(){
  ...
  this.remittanceService.lockForApprovalRemittance(this.oids, this.userInfo.loginId).subscribe(result => {
      this.params.searchText = null;
  ...
  goToMyLockList(){
    this.router.navigate(['/dashboard/my-approve-remittance-request'])
  }
  closeMsg(){
    this.msgs =[];
  }
